"""
figuras_convergencia_v2.py — Figuras de convergencia actualizadas (6 dp)

Incluye dp=0.004 (nuevo). Formato publicación, etiquetas limpias,
storytelling visual para presentación y paper.
...
# ═══════════════════════════════════════════════════════════════════════
# FIG 1: Displacement convergence (the main result)
# ═══════════════════════════════════════════════════════════════════════
...
# Annotations: delta% between successive points, positioned ABOVE the midpoint
...
# ═══════════════════════════════════════════════════════════════════════
# FIG 2: SPH Force convergence (the success story)
# ═══════════════════════════════════════════════════════════════════════
...
# Converged value band
ax.axhspan(22.0, 23.5, color=G, alpha=0.1, zorder=0)
ax.text(0.012, 21.0, '$F_{SPH}$ converged $\\approx$ 22.8 N',
...
# ═══════════════════════════════════════════════════════════════════════
# FIG 3: Convergence rate (delta% for all metrics)
# ═══════════════════════════════════════════════════════════════════════
...
ax.text(0.0045, 5.8, '5% convergence criterion', fontsize=7.5, color=GR)
...
ax.set_title('Convergence Rate — All Kinematic Metrics')
ax.invert_xaxis()
...
# 4a: Both forces on log scale
ax1.semilogy(dp, f_cont, 'o-', color=R, label='Contact (Chrono NSC)', zorder=5)
ax1.semilogy(dp, f_sph, 's-', color=B, label='SPH (fluid)', zorder=5)

# Label each point for contact
...
# 4b: Bar chart with resolution color coding
colors = []
for d in dim_dp:
    if d < 4:
...
bars = ax2.bar(range(len(dp)), f_cont, color=colors, edgecolor='black', lw=0.5, zorder=3)
...
for i, (bar, val) in enumerate(enumerate(f_cont)):
    val = f_cont[i]
...
fig.suptitle('Contact Force Non-Convergence — Resolution Sensitivity',
...
# ═══════════════════════════════════════════════════════════════════════
# FIG 5: Velocity + Rotation (stable metrics)
# ═══════════════════════════════════════════════════════════════════════
...
mean_vel = np.mean(vel_ms[2:])  # dp<=0.010
ax1.axhline(mean_vel, color=GR, ls=':', lw=0.8)
ax1.text(0.012, mean_vel + 0.008, f'Mean (dp$\\leq$0.01) = {mean_vel:.3f} m/s',
         fontsize=7.5, color=GR)

for i, (d, v) in enumerate(zip(dp, vel_ms)):
    yoff = 0.015 if v > mean_vel else -0.025
    ax1.annotate(f'{v:.3f}', xy=(d, v + yoff),
                 fontsize=7, ha='center', color=O)

ax1.set_xlabel('$dp$ [m]')
ax1.set_ylabel('Max boulder velocity [m/s]')
ax1.set_title('(a) Boulder Velocity — Converged')
ax1.invert_xaxis()
ax1.set_xlim(0.022, 0.003)
...
# Stable band for dp <= 0.008
ax2.axhspan(83, 88, color=P, alpha=0.06, zorder=0)
ax2.text(0.005, 89, 'Stabilizing ~85$\\degree$', fontsize=7.5,
         color=P, fontstyle='italic')

ax2.set_xlabel('$dp$ [m]')
ax2.set_ylabel('Max rotation [deg]')
ax2.set_title('(b) Boulder Rotation — Stabilizing')
ax2.invert_xaxis()
ax2.set_xlim(0.022, 0.003)
ax2.set_ylim(40, 110)

fig.suptitle('Kinematic Metrics — Velocity Converged, Rotation Stabilizing',
             fontsize=12, fontweight='bold', y=1.02)
fig.tight_layout()
...
# Stable band (dp 0.020-0.008)
mean_wh_stable = np.mean(water_m[:4])
ax.axhspan(mean_wh_stable - 0.01, mean_wh_stable + 0.01,
           color=B, alpha=0.08, zorder=0)
ax.axhline(mean_wh_stable, color=GR, ls=':', lw=0.8)
ax.text(0.018, mean_wh_stable + 0.012,
        f'Stable mean = {mean_wh_stable:.3f} m', fontsize=8, color=GR)

# Label each point
for i, (d, w) in enumerate(zip(dp, water_m)):
    yoff = 0.015 if w > 0.2 else -0.02
    ha = 'center'
    ax.annotate(f'{w:.3f} m', xy=(d, w + yoff),
                fontsize=7.5, ha=ha, color=B)
...
ax.set_title('Water Height at Gauge — Anomaly at Fine Resolution')
ax.invert_xaxis()
ax.set_xlim(0.022, 0.003)
ax.set_ylim(0.10, 0.40)

save(fig, 'fig06_water_height_anomaly')
...
fig.suptitle('Computational Cost — GPU Simulation on NVIDIA RTX 5090',
             fontsize=12, fontweight='bold', y=1.02)
fig.tight_layout()
save(fig, 'fig07_computational_cost')
...
# Row colors by resolution status
status_colors = {
    'UNDER-RES.': '#FFCDD2',
    'MARGINAL': '#FFE0B2',
    'TRANSITIONAL': '#FFF9C4',
...
fig.suptitle('Table 1: Mesh Convergence Results — Boulder BLIR3 ($d_{eq}$ = 10.0 cm, RTX 5090)',
             fontsize=11, fontweight='bold', y=0.95)
save(fig, 'fig08_summary_table')


# ═══════════════════════════════════════════════════════════════════════
# FIG 9: The "money figure" — complete story in one panel
# ═══════════════════════════════════════════════════════════════════════
...
ax.set_title('(d) Velocity [m/s] — CONVERGED', fontsize=10, color=G)
ax.set_xlabel('$dp$ [m]')
ax.invert_xaxis(); ax.set_xlim(0.022, 0.003)
...
ax.set_title('(e) Rotation [deg] — STABILIZING', fontsize=10, color=O)
ax.set_xlabel('$dp$ [m]')
ax.invert_xaxis(); ax.set_xlim(0.022, 0.003)
ax.set_ylim(40, 110)
...
fig.suptitle('Mesh Convergence Study — 6 Resolutions, RTX 5090, Boulder BLIR3',
...
print("\n" + "=" * 60)
print("CONVERGENCE STORY (6 dp)")
print("=" * 60)
print(f"\n{'dp':>6} {'Disp':>7} {'d%':>6} {'Rot':>6} {'Vel':>6} {'Fsph':>7} {'Fcont':>7} {'dim/dp':>6}")
print("-" * 60)
for i in range(len(dp)):
    d_str = f'{delta_disp[i-1]:.1f}%' if i > 0 else '—'
    print(f"{dp[i]:>6.3f} {disp_m[i]:>7.3f} {d_str:>6} {rot_deg[i]:>6.1f} "
          f"{vel_ms[i]:>6.3f} {f_sph[i]:>7.1f} {f_cont[i]:>7.0f} {dim_dp[i]:>6.0f}")

print(f"\nKEY FINDINGS:")
print(f"  Displacement delta: 28.3% -> 6.4% (IMPROVING)")
print(f"  SPH Force delta:    0.9% (CONVERGED)")
print(f"  Velocity delta:     0.8% (CONVERGED)")
print(f"  Rotation delta:     2.3% (STABILIZING)")
print(f"  Contact force:      ERRATIC (not convergent)")
print(f"  Water height:       ANOMALOUS at dp<=0.005")
print(f"\nANOMALIES:")
print(f"  1. Rotation dip at dp=0.010 (60.3 vs ~87)")
print(f"  2. Water height spike at dp=0.005 (0.320 vs ~0.16)")
print(f"  3. Water height elevated at dp=0.004 (0.245 vs ~0.16)")
print(f"  4. Contact force erratic across all dp (131-4915 N)")
print(f"\nWAITING: dp=0.003 (running, est. ~5-7h)")
